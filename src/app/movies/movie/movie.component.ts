...
export class MovieComponent implements OnInit {
  ...
  cast: MovieCastModel[]| undefined;
  crew: MovieCrewModel[]| undefined;
  isConnected = false;
  ...
  getCategories: Array<any>| undefined;
  categories : string[] = [];
  lang: string | null;
  ...
  pushMovieCategoryDefault(movie: MovieDatabaseModel, category: string) {
    this.databaseService.addMovieCategoriesDefault(movie, category, (error: string) => {
      if (error) {
  ...
  pushMovieCategory(movie: any, category: string) {
    this.databaseService.addMovieCategory(movie, category, (error: string) => {
      if (error) {
        this.snackBar.open(error, 'Hide', { duration: 5000 });
      } else {
        this.translateService.get('Error.Movie-added').subscribe(results => this.snackBar.open(results, '', { duration: 2000 }));
