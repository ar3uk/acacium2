...
export class CategoriesComponent implements OnInit, OnDestroy {
  ...
  deleteMovieFromFavorites(id: number, event : any) {
    event.stopPropagation();
    this.databaseService.deleteMoviesCategoriesDefault('FavoriteMovie', id, (error : any) => {
        if (error) {
  ...
  deleteMovieFromCategory(category: string, id: number) {
    this.databaseService.deleteMovieCategory(category, id, (error : any) => {
      if (error) {
        this.snackBar.open(error, 'Hide', { duration: 5000 });
      } else {
        this.translateService.get('Error.List-updated').subscribe(results => this.snackBar.open(results, '', { duration: 2000 }));
  ...
  addCategoryDialog() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.databaseService.addCategories(result, (error : any) => {
          if (error) {
            this.snackBar.open(error, 'hide', { duration: 5000});
          } else {
            this.translateService.get('Error.List-updated').subscribe(results => this.snackBar.open(results, '', { duration: 2000 }));
  ...
  deleteCategoryDialog() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.databaseService.deleteCategories(result, (error : any) => {
          if (error) {
            this.snackBar.open(error, 'hide', { duration: 5000});
          } else {
            this.translateService.get('Error.List-updated').subscribe(results => this.snackBar.open(results, '', { duration: 2000 }));
